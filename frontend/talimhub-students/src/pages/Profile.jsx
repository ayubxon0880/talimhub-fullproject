  ...
  useEffect(() => {
    const fetchUserData = async () => {
      ...
      try {
        ...
        const summaryRes = await axios.get(`${API}/user/details`, {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
          },
        });
      ...
      {loading ? (
        ...
        <div className="bg-white shadow-lg rounded-lg p-6 w-full max-w-md text-center">
          ...
          <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
            ...
            {userSummary && (
              <div className="mt-4">
                <p className="text-gray-600">Speakinglar soni: {userSummary.totalSpeakingCount}</p>
                <p className="text-gray-600">Umumiy likelar: {userSummary.totalLikeCount}</p>
              </div>
            )}
