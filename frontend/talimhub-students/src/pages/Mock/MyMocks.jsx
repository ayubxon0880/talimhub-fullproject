  ...
  const fetchMocks = async (page, pageSize, sorted) => {
    try {
      const response = await axios.get(
        `${API}/mock/my-mocks?page=${page}&size=${pageSize}&month=${selectedMonth.value}&checked=true`, {
          headers:{
...
{mocks.length === 0 ? (
  ...
  mocks.map((mock) => (
    ...
    <div key={mock.id} className="w-full md:w-96 lg:w-2/3 xl:w-1/2 mx-auto my-4 border-b rounded border p-4 transition-all hover:shadow-lg">
      <a href={`/mock-feedback/${mock.id}`} className="text-gray-700 text-left">
      ...
      <div className="text-right mt-2 flex justify-end items-center">
        <div className="flex space-x-2">
          <a
            ...
            className={`px-3 py-1 rounded-full bg-${mock.checked ? "blue" : "red"}-500 text-white transition-colors hover:bg-blue-600`}
          >
            Full view
          </a>
        </div>
